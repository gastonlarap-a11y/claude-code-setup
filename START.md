...
3. **Owner + global only** — is this a personal machine or a company/restricted one?
   - `secrets.env` is optional today (no keyed MCP servers in `global/mcp-servers.json`).
     Only when a future server declares env keys and the user HAS the values: read
     `secrets.env.example`, ask per variable, and write `secrets.env` yourself before
     running the installer; without the values, install anyway — those servers just
     register keyless.
...
| Who | Scope | Do this |
|---|---|---|
| Owner | Global | Follow `AGENT-INSTALL.md` (runs `install.sh`; on native Windows, `install.ps1` from PowerShell). `secrets.env` is optional today — only interview for it when `global/mcp-servers.json` declares env keys. |
...
- Global: run the checklist in `AGENT-INSTALL.md` section 3 (MCP connected, marketplace
  ...
  `.env` read denied). `claude mcp list` is empty today — `global/mcp-servers.json`
  ships no servers; that is the expected result, not a failure.
...
- Report in the user's language: what was configured, what was skipped and why, and the
  exact next command for anything deferred (e.g. adding a future MCP key later:
